...
tape( 'the function returns the standard deviation of a Bradford distribution given small parameter `c`', opts, function test( t ) {
	var expected;
	var i;
	var c;
	var y;
	...
	for ( i = 0; i < expected.length; i++ ) {
		...
		*/
		t.strictEqual( isAlmostSameValue( y, expected[ i ], 371 ), true, 'returns expected value' );
	}
	t.end();
});

tape( 'the function returns the standard deviation of a Bradford distribution given large parameter `c`', opts, function test( t ) {
	var expected;
	var i;
	var c;
	var y;
